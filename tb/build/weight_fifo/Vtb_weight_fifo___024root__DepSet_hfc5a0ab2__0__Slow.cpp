// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design implementation internals
// See Vtb_weight_fifo.h for the primary calling header

#include "Vtb_weight_fifo__pch.h"
#include "Vtb_weight_fifo___024root.h"

VL_ATTR_COLD void Vtb_weight_fifo___024root___eval_static__TOP(Vtb_weight_fifo___024root* vlSelf);

VL_ATTR_COLD void Vtb_weight_fifo___024root___eval_static(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___eval_static\n"); );
    // Body
    Vtb_weight_fifo___024root___eval_static__TOP(vlSelf);
}

VL_ATTR_COLD void Vtb_weight_fifo___024root___eval_static__TOP(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___eval_static__TOP\n"); );
    // Body
    vlSelf->tb_weight_fifo__DOT__errors = 0U;
    vlSelf->tb_weight_fifo__DOT__checks = 0U;
    vlSelf->tb_weight_fifo__DOT__clk = 0U;
    vlSelf->tb_weight_fifo__DOT__fifo_test_done = 0U;
    vlSelf->tb_weight_fifo__DOT__wl_test_done = 0U;
    vlSelf->tb_weight_fifo__DOT__clk2 = 0U;
    vlSelf->tb_weight_fifo__DOT__wl_errors = 0U;
    vlSelf->tb_weight_fifo__DOT__wl_checks = 0U;
}

VL_ATTR_COLD void Vtb_weight_fifo___024root___eval_final(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___eval_final\n"); );
}

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtb_weight_fifo___024root___dump_triggers__stl(Vtb_weight_fifo___024root* vlSelf);
#endif  // VL_DEBUG
VL_ATTR_COLD bool Vtb_weight_fifo___024root___eval_phase__stl(Vtb_weight_fifo___024root* vlSelf);

VL_ATTR_COLD void Vtb_weight_fifo___024root___eval_settle(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___eval_settle\n"); );
    // Init
    IData/*31:0*/ __VstlIterCount;
    CData/*0:0*/ __VstlContinue;
    // Body
    __VstlIterCount = 0U;
    vlSelf->__VstlFirstIteration = 1U;
    __VstlContinue = 1U;
    while (__VstlContinue) {
        if (VL_UNLIKELY((0x64U < __VstlIterCount))) {
#ifdef VL_DEBUG
            Vtb_weight_fifo___024root___dump_triggers__stl(vlSelf);
#endif
            VL_FATAL_MT("tb/tb_weight_fifo.sv", 14, "", "Settle region did not converge.");
        }
        __VstlIterCount = ((IData)(1U) + __VstlIterCount);
        __VstlContinue = 0U;
        if (Vtb_weight_fifo___024root___eval_phase__stl(vlSelf)) {
            __VstlContinue = 1U;
        }
        vlSelf->__VstlFirstIteration = 0U;
    }
}

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtb_weight_fifo___024root___dump_triggers__stl(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___dump_triggers__stl\n"); );
    // Body
    if ((1U & (~ (IData)(vlSelf->__VstlTriggered.any())))) {
        VL_DBG_MSGF("         No triggers active\n");
    }
    if ((1ULL & vlSelf->__VstlTriggered.word(0U))) {
        VL_DBG_MSGF("         'stl' region trigger index 0 is active: Internal 'stl' trigger - first iteration\n");
    }
}
#endif  // VL_DEBUG

VL_ATTR_COLD void Vtb_weight_fifo___024root___stl_sequent__TOP__0(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___stl_sequent__TOP__0\n"); );
    // Body
    vlSelf->tb_weight_fifo__DOT__empty = (0U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__count));
    vlSelf->tb_weight_fifo__DOT__full = (4U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__count));
    vlSelf->tb_weight_fifo__DOT__fifo_empty = (0U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count));
    vlSelf->tb_weight_fifo__DOT__fifo_full = (4U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count));
    vlSelf->tb_weight_fifo__DOT__data_out[0U] = 0U;
    vlSelf->tb_weight_fifo__DOT__data_out[1U] = 0U;
    vlSelf->tb_weight_fifo__DOT__data_out[2U] = 0U;
    if ((0U != (IData)(vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__count))) {
        vlSelf->tb_weight_fifo__DOT__data_out[0U] = 
            vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__Q
            [(3U & ((IData)(vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__count) 
                    - (IData)(1U)))][0U];
        vlSelf->tb_weight_fifo__DOT__data_out[1U] = 
            vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__Q
            [(3U & ((IData)(vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__count) 
                    - (IData)(1U)))][1U];
        vlSelf->tb_weight_fifo__DOT__data_out[2U] = 
            vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__Q
            [(3U & ((IData)(vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__count) 
                    - (IData)(1U)))][2U];
    }
    vlSelf->tb_weight_fifo__DOT__wl_data_out = 0U;
    if ((0U != (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count))) {
        vlSelf->tb_weight_fifo__DOT__wl_data_out = 
            vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__Q
            [(0x1fU & ((IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count) 
                       - (IData)(1U)))];
    }
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__fifo_re = 0U;
    vlSelf->tb_weight_fifo__DOT__data_valid = 0U;
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__fifo_we = 0U;
    if ((0U != (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state))) {
        if ((1U != (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state))) {
            if ((2U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state))) {
                if (((0U != (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count)) 
                     & (IData)(vlSelf->tb_weight_fifo__DOT__preload_state))) {
                    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__fifo_re = 1U;
                    vlSelf->tb_weight_fifo__DOT__data_valid = 1U;
                }
            }
        }
        if ((1U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state))) {
            if (((4U != (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count)) 
                 & (IData)(vlSelf->tb_weight_fifo__DOT__load_fifo_state))) {
                if (vlSelf->tb_weight_fifo__DOT__read_data_valid) {
                    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__fifo_we = 1U;
                }
            }
        }
    }
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__next_state 
        = vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state;
    if ((0U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state))) {
        vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__next_state 
            = (((IData)(vlSelf->tb_weight_fifo__DOT__start_load_fifo_state) 
                | (IData)(vlSelf->tb_weight_fifo__DOT__tiles_complete))
                ? 1U : 0U);
    } else if ((1U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state))) {
        vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__next_state 
            = (((4U != (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count)) 
                & (IData)(vlSelf->tb_weight_fifo__DOT__load_fifo_state))
                ? 1U : 2U);
    } else if ((2U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state))) {
        vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__next_state 
            = (((0U != (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count)) 
                & (IData)(vlSelf->tb_weight_fifo__DOT__preload_state))
                ? 2U : ((0U == (IData)(vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count))
                         ? 0U : 2U));
    }
}

VL_ATTR_COLD void Vtb_weight_fifo___024root___eval_stl(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___eval_stl\n"); );
    // Body
    if ((1ULL & vlSelf->__VstlTriggered.word(0U))) {
        Vtb_weight_fifo___024root___stl_sequent__TOP__0(vlSelf);
    }
}

VL_ATTR_COLD void Vtb_weight_fifo___024root___eval_triggers__stl(Vtb_weight_fifo___024root* vlSelf);

VL_ATTR_COLD bool Vtb_weight_fifo___024root___eval_phase__stl(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___eval_phase__stl\n"); );
    // Init
    CData/*0:0*/ __VstlExecute;
    // Body
    Vtb_weight_fifo___024root___eval_triggers__stl(vlSelf);
    __VstlExecute = vlSelf->__VstlTriggered.any();
    if (__VstlExecute) {
        Vtb_weight_fifo___024root___eval_stl(vlSelf);
    }
    return (__VstlExecute);
}

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtb_weight_fifo___024root___dump_triggers__act(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___dump_triggers__act\n"); );
    // Body
    if ((1U & (~ (IData)(vlSelf->__VactTriggered.any())))) {
        VL_DBG_MSGF("         No triggers active\n");
    }
    if ((1ULL & vlSelf->__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 0 is active: @(posedge tb_weight_fifo.clk or negedge tb_weight_fifo.reset_n)\n");
    }
    if ((2ULL & vlSelf->__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 1 is active: @(posedge tb_weight_fifo.clk2 or negedge tb_weight_fifo.rst_n2)\n");
    }
    if ((4ULL & vlSelf->__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 2 is active: @(posedge tb_weight_fifo.clk)\n");
    }
    if ((8ULL & vlSelf->__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 3 is active: @(negedge tb_weight_fifo.clk)\n");
    }
    if ((0x10ULL & vlSelf->__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 4 is active: @([true] __VdlySched.awaitingCurrentTime())\n");
    }
    if ((0x20ULL & vlSelf->__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 5 is active: @(posedge tb_weight_fifo.clk2)\n");
    }
    if ((0x40ULL & vlSelf->__VactTriggered.word(0U))) {
        VL_DBG_MSGF("         'act' region trigger index 6 is active: @([changed] (tb_weight_fifo.fifo_test_done & tb_weight_fifo.wl_test_done))\n");
    }
}
#endif  // VL_DEBUG

#ifdef VL_DEBUG
VL_ATTR_COLD void Vtb_weight_fifo___024root___dump_triggers__nba(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___dump_triggers__nba\n"); );
    // Body
    if ((1U & (~ (IData)(vlSelf->__VnbaTriggered.any())))) {
        VL_DBG_MSGF("         No triggers active\n");
    }
    if ((1ULL & vlSelf->__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 0 is active: @(posedge tb_weight_fifo.clk or negedge tb_weight_fifo.reset_n)\n");
    }
    if ((2ULL & vlSelf->__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 1 is active: @(posedge tb_weight_fifo.clk2 or negedge tb_weight_fifo.rst_n2)\n");
    }
    if ((4ULL & vlSelf->__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 2 is active: @(posedge tb_weight_fifo.clk)\n");
    }
    if ((8ULL & vlSelf->__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 3 is active: @(negedge tb_weight_fifo.clk)\n");
    }
    if ((0x10ULL & vlSelf->__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 4 is active: @([true] __VdlySched.awaitingCurrentTime())\n");
    }
    if ((0x20ULL & vlSelf->__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 5 is active: @(posedge tb_weight_fifo.clk2)\n");
    }
    if ((0x40ULL & vlSelf->__VnbaTriggered.word(0U))) {
        VL_DBG_MSGF("         'nba' region trigger index 6 is active: @([changed] (tb_weight_fifo.fifo_test_done & tb_weight_fifo.wl_test_done))\n");
    }
}
#endif  // VL_DEBUG

VL_ATTR_COLD void Vtb_weight_fifo___024root___ctor_var_reset(Vtb_weight_fifo___024root* vlSelf) {
    if (false && vlSelf) {}  // Prevent unused
    Vtb_weight_fifo__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+    Vtb_weight_fifo___024root___ctor_var_reset\n"); );
    // Body
    vlSelf->tb_weight_fifo__DOT__errors = 0;
    vlSelf->tb_weight_fifo__DOT__checks = 0;
    vlSelf->tb_weight_fifo__DOT__clk = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__reset_n = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__we = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__re = VL_RAND_RESET_I(1);
    VL_RAND_RESET_W(96, vlSelf->tb_weight_fifo__DOT__data_in);
    VL_RAND_RESET_W(96, vlSelf->tb_weight_fifo__DOT__data_out);
    vlSelf->tb_weight_fifo__DOT__full = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__empty = VL_RAND_RESET_I(1);
    for (int __Vi0 = 0; __Vi0 < 4; ++__Vi0) {
        VL_RAND_RESET_W(96, vlSelf->tb_weight_fifo__DOT__wr_word[__Vi0]);
    }
    vlSelf->tb_weight_fifo__DOT__fifo_test_done = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__wl_test_done = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__clk2 = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__rst_n2 = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__read_data = VL_RAND_RESET_I(32);
    vlSelf->tb_weight_fifo__DOT__start_load_fifo_state = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__load_fifo_state = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__preload_state = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__tiles_complete = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__read_data_valid = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__fifo_full = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__fifo_empty = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__data_valid = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__wl_data_out = VL_RAND_RESET_I(32);
    vlSelf->tb_weight_fifo__DOT__wl_errors = 0;
    vlSelf->tb_weight_fifo__DOT__wl_checks = 0;
    for (int __Vi0 = 0; __Vi0 < 4; ++__Vi0) {
        VL_RAND_RESET_W(96, vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__Q[__Vi0]);
    }
    vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__putPtr = VL_RAND_RESET_I(2);
    vlSelf->tb_weight_fifo__DOT__dut_fifo__DOT__count = VL_RAND_RESET_I(4);
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__fifo_re = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__fifo_we = VL_RAND_RESET_I(1);
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__current_state = VL_RAND_RESET_I(5);
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__next_state = VL_RAND_RESET_I(5);
    for (int __Vi0 = 0; __Vi0 < 32; ++__Vi0) {
        vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__Q[__Vi0] = VL_RAND_RESET_I(32);
    }
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__putPtr = VL_RAND_RESET_I(2);
    vlSelf->tb_weight_fifo__DOT__dut_wl__DOT__weight_fifo__DOT__count = VL_RAND_RESET_I(4);
    vlSelf->__Vtrigprevexpr___TOP__tb_weight_fifo__DOT__clk__0 = VL_RAND_RESET_I(1);
    vlSelf->__Vtrigprevexpr___TOP__tb_weight_fifo__DOT__reset_n__0 = VL_RAND_RESET_I(1);
    vlSelf->__Vtrigprevexpr___TOP__tb_weight_fifo__DOT__clk2__0 = VL_RAND_RESET_I(1);
    vlSelf->__Vtrigprevexpr___TOP__tb_weight_fifo__DOT__rst_n2__0 = VL_RAND_RESET_I(1);
    vlSelf->__Vtrigprevexpr_h9cca9006__0 = VL_RAND_RESET_I(1);
    vlSelf->__VactDidInit = 0;
}
